        ...
        try:
            ...
            for o in iterator:
                if o not in self._observers:
                    ...
                    self.logger.debug(f'Failed to add {o} to event observers')
        except TypeError:
            if observer not in self._observers:
                self._observers.append(observer)
            else:
                self.logger.debug(f'Failed to add {observer} to event observers')
        ...
        try:
            ...
            for o in iterator:
                try:
                    ...
                    self.logger.debug(f'Failed to remove {o} from event observers')
        except TypeError:
            try:
                self._observers.remove(observer)
            except ValueError:
                self.logger.debug(f'Failed to remove {observer} from event observers')
